...
VOID ReadRegister(BYTE uaddr, Bme280Register ureg, VOID* pbuf, UINT usize)
{
auto hi2c=I2CBus::Current;
hi2c->BeginWrite(uaddr);
hi2c->Write((BYTE)ureg);
hi2c->EndTransmission();
hi2c->BeginRead(uaddr);
hi2c->Read(pbuf, usize);
hi2c->EndTransmission();
}

VOID WriteRegister(BYTE uaddr, Bme280Register ureg, VOID const* pbuf, UINT usize)
{
auto hi2c=I2CBus::Current;
hi2c->BeginWrite(uaddr);
hi2c->Write((BYTE)ureg);
hi2c->Write(pbuf, usize);
hi2c->EndTransmission();
}
...
BYTE uid=0;
ReadRegister(cSettings.I2cAddress, Bme280Register::Id, &uid, 1);
if(uid!=0x60)
	{
...
BYTE pbuf[4];
ReadRegister(cSettings.I2cAddress, Bme280Register::HumidityControl, pbuf, 4);
...
if(cSettings.Mode==Bme280Mode::Normal)
	{
	hTimer=new Core::Timer();
	hTimer->Triggered.Add(this, &Bme280::OnTimerTriggered);
	UINT utime=StandbyToMillis(cSettings.Standby);
	hTimer->StartPeriodic(utime);
	}
}
...
if(cSettings.Mode==Bme280Mode::Forced)
	WriteDeviceSettings();
BYTE pdata[8];
ReadRegister(cSettings.I2cAddress, Bme280Register::Data, pdata, 8);
UINT rawt=((UINT)pdata[3]<<12)|((UINT)pdata[4]<<4)|(pdata[5]>>4);
UINT rawp=((UINT)pdata[0]<<12)|((UINT)pdata[1]<<4)|(pdata[2]>>4);
UINT rawh=((UINT)pdata[6]<<8)|pdata[7];
INT ifine=0;
FLOAT ftemp=CalculateTemperature(rawt, ifine);
FLOAT fhum=CalculateHumidity(rawh, ifine);
FLOAT fpress=CalculatePressure(rawp, ifine);
Humidity->Set(fhum);
Pressure->Set(fpress);
...
VOID Bme280::OnTimerTriggered(Handle<Core::Timer> htimer)
{
Update();
}
